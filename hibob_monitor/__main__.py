...
def run_hibob_monitor(domain: str, browser: str, format_type: str, output_path: Optional[str] = None, 
                      enable_change_tracking: bool = True, cache_file: str = 'data/employees_cache.json', 
                      log_file: str = 'data/employee_changes.log') -> None:
    """Main application logic with change tracking."""
    print(f"🔍 HiBob Employee Monitor (Modular Functional Version)")
    print(f"📍 Domain: {domain}")
    print(f"🌐 Browser: {browser.title()}")
    if enable_change_tracking:
        print(f"📝 Change tracking: enabled (cache: {cache_file}, log: {log_file})")
    else:
        print(f"📝 Change tracking: disabled")
    print()
    ...
    if enable_change_tracking:
        ...
        if previous_employee_list is not None:
            ...
            if change_report.has_changes:
                print(f"📈 Changes detected: {len(change_report.added)} added, {len(change_report.removed)} removed, {len(change_report.modified)} modified")
