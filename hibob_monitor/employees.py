...
from .config import API_ENDPOINTS, EMPLOYEE_DATA_KEYS, EMPLOYEE_FIELDS, ACTIVE_STATUS_VALUES
...
def _filter_active_employees(employees: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for emp in inactive_employees:
        print(f"⚠️  Employee {emp.get('id', 'unknown')} is not active (status: {str(emp.get('status'))})")
    
    return active_employees


def _try_endpoint(base_url: str, endpoint: str, cookies: Dict[str, str]) -> Optional[EmployeeList]:
    ...
    if data is not None:
        ...
        if employees_data:
            ...
            if active_employees_data:
                ...
                print(f"⚠️  Found {len(employees_data)} employees but none marked as active")
    
    return None


def get_active_employees(base_url: str, cookies: Dict[str, str]) -> Optional[EmployeeList]:
    """Fetch list of active employees by trying multiple endpoints."""
    endpoint_functions = [partial(_try_endpoint, base_url, endpoint, cookies) for endpoint in API_ENDPOINTS]
