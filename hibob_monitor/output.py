...
def _get_display_fields(employee_list: EmployeeList) -> List[str]:
    ...
    all_fields = set(chain.from_iterable(emp.raw_data.keys() for emp in employee_list.employees))
    
    # Prioritize important fields, including nested fields
    fields_to_show = [f for f in PRIORITY_DISPLAY_FIELDS if f.split(".")[0] in all_fields]
...
def print_table(employee_list: EmployeeList) -> None:
    ...
    column_widths = {
        field: max(len(field), 15) for field in fields_to_show
    }
...
def save_to_json(employee_list: EmployeeList, filepath: str) -> None:
    ...
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump([emp.raw_data for emp in employee_list.employees], f, indent=2, ensure_ascii=False)
...
def save_to_csv(employee_list: EmployeeList, filepath: str) -> None:
    ...
    try:
        ...
        all_fields = sorted(set(chain.from_iterable(emp.raw_data.keys() for emp in employee_list.employees)))
...
def handle_output(employee_list: EmployeeList, format_type: str, output_path: Optional[str]) -> None:
    """Handle output based on format and output path."""
    output_handlers = {
        OutputFormats.JSON.value: (
            save_to_json, 
            lambda el: print(json.dumps([emp.raw_data for emp in el.employees], indent=2, ensure_ascii=False))
        ),
